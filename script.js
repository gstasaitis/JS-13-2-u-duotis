...
// Funkcijos su array metodais. Naudodami array metodais per console atspausdinkite:
// 1. Kiek vyrų yra tarp šių duomenų

const filterByGender = () => {
    const genderFilter = data.filter(x => x.gender == "Male")
    return genderFilter
}
console.log(filterByGender())

// 2. Sukurti masyvą, kuriame id žmonių, kurių automobiliai naujesni nei 2000

const richBitches = () => {
    const filterByRich = data.filter(x => x.car_year > 2000)
    return filterByRich
}
console.log(richBitches())

// 3. Sukurti masyvą, kuriame pagal abėcėlę būtų surūšiuoti vardai žmonių, kurių dydžiai XS arb S

const abcdLSD = () => {
    const nameSort = data.sort((a , b) => a.first_name > b.first_name ? 1 : -1)
    const filterBySize = nameSort.filter(x => x.shirt_size == "XS" || x.shirt_size == "S").sort((a , b) => a.first_name > b.first_name)
...
console.log(abcdLSD().map(x => ({id: x.id, first_name:x.first_name, shirt_size: x.shirt_size})))